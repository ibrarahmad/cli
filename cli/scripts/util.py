...
def run_rcommand(cmd, message="", host="", usr="", key="", verbose=False, max_attempts=1, capture_output=False):
    if message == "":
        return echo_rcmd(cmd, echo=verbose, host=host, usr=usr, key=key, capture_output=capture_output)
    if host == "":
        host = "127.0.0.1"
    ...
    while attempts < max_attempts:
        result = echo_rcmd(cmd, echo=verbose, host=host, usr=usr, key=key, capture_output=True)
